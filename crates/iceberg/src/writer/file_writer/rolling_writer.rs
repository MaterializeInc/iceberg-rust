...
use crate::spec::{DataFileBuilder, PartitionKey, SchemaRef, TableProperties};
...
mod tests {
    ...
    #[tokio::test]
    async fn test_current_file_status_before_first_write() -> Result<()> {
        // CurrentFileStatus methods must not panic before the inner writer is
        // lazily initialized on first write. DeltaWriter::try_new relies on
        // current_schema() here.
        let temp_dir = TempDir::new()?;
        let file_io = FileIO::new_with_fs();
        let location_gen = DefaultLocationGenerator::with_data_location(
        ...
        assert!(Arc::ptr_eq(&rolling.current_schema(), &schema));
        assert_eq!(rolling.current_row_num(), 0);
        assert_eq!(rolling.current_written_size(), 0);
